...
public class UserTests {
    ...
    public void testGetUserByName(){
        ...
        Response response = UseEndPoints.getUser(this.userPayload.getUsername());
        response.then().log().all();
        Assert.assertEquals(response.getStatusCode(),200);
    ...
    public void testUpdateUserByName(){
        ...
        userPayload.setLastName(faker.name().lastName());
        userPayload.setEmail(faker.internet().safeEmailAddress());
        Response response = UseEndPoints.updateUser(this.userPayload.getUsername(),userPayload);
        response.then().log().body().statusCode(200);
        logger.info("SuccessFully update details user");


        //checking data update or not
        Response responseAfterUpdate = UseEndPoints.getUser(this.userPayload.getUsername());
        response.then().log().all();
    ...
    public void testDeleteUserByName(){
        ...
        Response response =  UseEndPoints.deleteUser(this.userPayload.getUsername());
        response.then().log().all();
        Assert.assertEquals(response.getStatusCode(),200);
